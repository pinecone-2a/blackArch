...
export default function CategoryFilterSide() {
  ...
  return (
    <div
      className="max-w-[375px] w-full border-2 rounded-3xl my-6 p-5 mx-auto 
        sm:w-[90%] md:w-[80%] lg:w-[60%] xl:w-[40%] transition-all duration-300 ease-in-out overflow-hidden"
      style={{ height: isFilterVisible ? "auto" : "70px" }}>
      <div className="flex items-center justify-between mb-4">
        <h1 className="font-bold text-2xl">Filters</h1>
        <SlidersHorizontal className="cursor-pointer" onClick={toggleFilterVisibility} />
      </div>
      {isFilterVisible && (
        <>
          ...
          <div>
            ...
            {openSections.colors && (
              <div className="flex gap-2 flex-wrap mt-3">
                {["#8B4513", "#8B0000", "#2E8B57", "#000000", "#4682B4", "#DAA520"].map((color, index) => (
                  <div
                    key={index}
