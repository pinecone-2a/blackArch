      ...
      {cart.length > 0 ? (
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="lg:w-2/3 space-y-4 w-full">
              {cart.map((item: CartItem) => (
                <Card key={item.productId} className="flex flex-col sm:flex-row items-center p-4 sm:p-6 gap-6 rounded-xl border shadow-sm hover:shadow-md transition-shadow duration-200 relative w-full group">
                  <div className="bg-gray-50 rounded-lg p-2 flex-shrink-0">
                    <img src={item.image} alt={item.name} className="w-24 h-24 sm:w-28 sm:h-28 object-contain" />
                  </div>
                  ...
                  <CardContent className="flex-1 text-center sm:text-left p-0 pt-2 sm:p-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:gap-4 mt-1 text-gray-600">
                      <p className="text-sm">{item.size ? `Size: ${item.size}` : ''}</p>
                      <p className="text-sm">{item.color ? `Color: ${item.color}` : ''}</p>
                    </div>
              ...
              <div className="bg-white border shadow-sm p-6 rounded-xl sticky top-28">
                ...
                <div className="mt-6 p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center">
                    ...
                    />
                    <Button onClick={() => setDiscount(promoCode === "Pineshop" ? 0.2 : 0)} variant="outline" size="sm">
                      Apply
                    </Button>
                  </div>
                </div>
                <p className="text-xs text-gray-500 mt-2">Use promo code <b>Pineshop</b> for 20% discount</p>
        ...
        <div className="flex flex-col items-center justify-center py-24">
          <div className="bg-white border rounded-xl shadow-sm p-10 max-w-md w-full text-center">
            ...
            <p className="mt-3 text-gray-500 mb-8">Looks like you haven't added anything to your cart yet.</p>
            <Link href="/category">
