  ...
  return (
    <div className=" gap-y-10 flex-wrap flex justify-between">
      <div className="flex flex-wrap gap-3">
        <div className="bg-gray-100 cart:block flex justify-center items-center cart:flex-shrink-0 flex-grow h-fit">
          <Img
            className="cart:w-32 cart:h-32"
            src={item.src}
            img={item.src?.split("/").pop()?.split(".")[0] || ""}
            alt=""
            from="cart"
          />
        </div>
        <div className="flex flex-col justify-between">
          <div className="flex flex-col">
            <h1
              className="font-bold text-2xl overflow-hidden w-[190px]"
              style={{ textOverflow: "ellipsis", whiteSpace: "nowrap" }}
              onClick={() => setShowName((pre) => !pre)}
              title={item.name}
            >
              {item.name}
            </h1>
            <AnimatePresence>
              {ShowName && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="absolute text-xs bg-black text-white p-2"
                >
                  {item.name}
                </motion.div>
              )}
      ...
      <div className="relative flex-grow cart2:flex-grow-0 items-end flex flex-col justify-end">
        <FaRegTrashAlt
          ...
          size={20}
          color="red"
          className="cursor-pointer absolute -top-10 cart2:top-0 right-0 font-extrabold "
        />
        <div className="rounded-full w-full select-none xsm:p-0 flex justify-center items-center gap-5 bg-gray-100">
          <span
            onClick={() => handleQuantityChange(-1)}
            className="text-4xl -mt-1 px-3 py-1 flex-grow cursor-pointer text-center"
          >
            -
          </span>
