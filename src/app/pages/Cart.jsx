    ...
    return (
      <div className="mainMargin flex-col-reverse lg:flex-row flex items-center">
        <div className="text-center flex flex-col items-center gap-5">
          <Reavel>
            <h1 className="bolded text-4xl xsm:text-5xl">
          ...
          <Reavel className="p-4">
            <Link className="btn w-fit" to="/Shop">
              Go to Shop
            </Link>
          </Reavel>
        </div>
  ...
  return (
    <div className="mainMargin">
      ...
      <div className="mt-10 flex flex-wrap gap-5">
        <motion.div
          key={Cart.length}
          className="flex flex-grow-huge h-fit flex-col rounded-xl border-2 p-5"
        >
          <AnimatePresence>
            {Cart.map((item, index) => {
              return (
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex justify-between">
              ...
              <div className="text-lg font-bold">
                ${15 + Cost - (Cost * Discount) / 100}
              </div>
            </div>
            <div>
              <div
                title="Moemen"
                className="absolute translate-y-4 translate-x-4"
              >
              ...
              <form onSubmit={handlePromoCode} className="flex flex-wrap gap-5">
                <input
                  ref={DiscountRef}
                  className="flex-grow bg-gray-100 rounded-full py-3 pl-10 outline-none"
                  placeholder="Add promo code"
                  type="text"
                />
                <button
                  className="btn text-center sm:flex-grow-0 flex-grow"
                  type="submit"
                >
                  Apply
                </button>
              ...
              {CorrectDiscount === false && (
                <p className="font-bold mt-5 text-center text-red-600">
                  Wrong promo code please try again!
                </p>
              )}
              {CorrectDiscount === "Yet" && (
                <p className="font-bold mt-5 text-center">
                  Use promo code Moemen for 20% discount
                </p>
              )}
              {CorrectDiscount === true && (
                <p className="font-bold mt-5 text-center text-green-600">
            ...
            <div className="flex mt-5 mb-5">
              <Link
                className="btn flex-grow text-center"
                onClick={() => {
                  AddToCart([], setCart, setCost, 0, "empty");
                  setCheckout(true);
                }}
